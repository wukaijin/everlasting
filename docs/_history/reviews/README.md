...
| [`REVIEW-remote-access-research-2026-07-20.md`](./REVIEW-remote-access-research-2026-07-20.md) | 设计评审 | MiniMax-M3 | 2026-07-20 | 远程访问/多通道改造调研评估稿评审; 综合 ★★★½; 发现 4 处 P0 数字错误(invoke 命令 57→80 / emit 事件 9→10 / 前端 22→21+8 listen / AppHandleSink 未覆盖 4 文件 emit 散点)+ 3 处 P1 设计遗漏(`apply_ui_diff` 错误形态误读 / SSE 缺 backpressure / Phase 2 dual-pool 写竞争)+ Transport.listen 语义错位建议; 路线图方向正确,补正后可进 Phase 1 |
| [`REVIEW-remote-access-tasks-2026-07-20.md`](./REVIEW-remote-access-tasks-2026-07-20.md) | 设计评审 | MiniMax-M3 | 2026-07-20 | 远程访问改造 Trellis Task 体系评审(parent + 2 child); 综合 ★★★★; 发现 3 处 P0(child 2 缺 design/implement + parent D1 未 ack + jsonl 空 stub)+ 3 处 P1(优先级标 P2 与 P0/P1 findings 不匹配 / `HttpSseSubagentSink` 跨 phase 接口未承接 / SSE R6 缺 lost segment 重发)+ 跨文档同步未明确 owner; triage 文档质量 ★★★★★ 单独记载; 补正后可正常 `task.py start` |

> 已移至 [`_deprecated/`](../deprecated/) 的早期评审快照见该目录 README。
