...
## 2. Schema 索引(15 张业务表 + 2 张 FTS5 虚拟表)
...
| 15 | `group_chat_presets` | `db/migrations/schema.rs`(尾部,幂等建表) | **09-12 GCE-P1**:用户群聊预设(Settings「群聊预设」页 CRUD,IPC 四条见 [DAEMON-API.md §6.4](./DAEMON-API.md)):`id` (TEXT PK,`Uuid::new_v4()`) / `name` (TEXT UNIQUE,显示名;大小写不敏感唯一在 commands 层校验) / `description` / `moderator_model_id` / `participants` (JSON 数组,camelCase `{name, modelId, persona}` 与 wire 同形) / `created_at` / `updated_at` / **`builtin_key`**(09-12 GCE-P1b,可空 TEXT;NULL = 普通用户行,值 ∈ 四内置 key = **覆盖行**,原位顶替对应内置预设;UNIQUE 索引 `idx_group_chat_presets_builtin_key` 保证每 key 至多一条 —— SQLite UNIQUE 对 NULL 互不相撞,普通行不受影响;update 不触碰该列,删除覆盖行 = 恢复内置)。模型引用是 models.id UUID 的 soft reference(无 FK,改名不断链;模型被删不级联,保存时校验存在/允许 disabled)。**内置四档(review/fe_review/arch/retro)本体不在此表** —— 仍是 `scripts/group-chat-presets.json` 单一事实源,M1/MCP 暂只认内置、不消费覆盖行(P2)。CRUD 在 `db/group_chat_presets.rs`;定时任务 `group_chat_config.preset_key` 可指向本表行 id 或内置 key(快照语义,fire 零读取) |
| 16 | `messages_fts` | `db/migrations/schema.rs` | FTS5 虚拟表(external-content + trigram tokenizer + `UPDATE OF text` 触发器防写放大 + `messages_fts_docsize` 影子表守卫回填)—— D2 跨 session 全文搜索的存储后端(2026-08-17);另有 `autonomous_memories_fts`(自主记忆自身搜索,连同 FTS shadow 表不逐一编号) |

**索引**:`idx_sessions_updated_at` / `idx_sessions_project_id` / `idx_messages_session_seq` / `idx_session_audit_events_session_ts` / `idx_subagent_runs_request` / `idx_am_pitfall`(autonomous_memories 的 `tool_name` 等 trigger 命中)/ `idx_autonomous_memories_status` / **`idx_session_audit_events_turn_seq`**(E2 07-14)/ **`idx_turn_trace_run`**(08-20,partial index `ON turn_trace(run_id) WHERE run_id != ''`,专服 `list_worker_turn_traces` 按 run_id 点查)/ **`idx_messages_status`**(08-24,partial index `ON messages(status) WHERE status IS NOT NULL`,启动恢复扫描专用)/ **`idx_scheduled_tasks_due`**(08-28,`ON scheduled_tasks(enabled, next_fire_at)`,调度器每 tick 扫描未到期任务)/ **`idx_group_chat_presets_builtin_key`**(09-12 GCE-P1b,UNIQUE,每内置 key 至多一条覆盖行;NULL 互不相撞)/ **FTS5 影子表 `messages_fts_docsize`** 索引(`db/migrations/schema.rs` 顶部)。
...
-- 10. GCE-P1b (2026-09-12) — 查内置档覆盖行(builtin_key 非空 = 覆盖行,
--      原位顶替 scripts/group-chat-presets.json 对应档;NULL = 普通用户行)
SELECT builtin_key, name, moderator_model_id, updated_at
