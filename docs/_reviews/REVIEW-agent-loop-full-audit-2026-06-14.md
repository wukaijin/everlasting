...
| **P2-2** | user_dir 路径与 Claude Code 实际路径不一致 ✅ **已修复 ([06-26-user-claude-md-home-dir](../_reviews/../IMPLEMENTATION.md))** | `file.rs:58-66` 用 `~/.config/everlasting/CLAUDE.md`;Claude Code 用户级在 `~/.claude/CLAUDE.md` | 用户从 Claude Code 切过来**用户层指令不共享**(项目层共享成立) |
...
- [x] user_dir 路径与 Claude Code `~/.claude/` 对齐决策(C-P2-2) — closed by 06-26-user-claude-md-home-dir
