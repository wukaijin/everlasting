...
| **C2+** 循环检测主动干预 | 07-06 | C2(06-24)软提示只注入 hint 不终止 loop,MAX_TURNS=200 是唯一硬兜底,死循环烧满 200 turn 敞口未堵。补中间主动层:**per-`run_chat_loop`-local `loop_hit_count`**(跟 `loop_window` 同生命周期,worker 继承),连续 3 轮 detect 命中 → 复用 `QuestionStore` + `emit_tool_question` + 前端 `<AskUserQuestionCard>` 在 chat_loop 顶层(harness-driven,非 LLM tool 路径)询问"是否终止"。`tokio::select!{token.cancelled(), oneshot}` 三分支:终止 → `Done{stop_reason="loop_terminated"}` / 继续 → count 清零 + 增强 hint 注入 / cancel → `Done{cancelled}`。Hard/Soft 共用 N=3 单一计数器。worker 触发走 `effective_is_worker` gate **直接 break**(不弹 banner,`dispatch_result` caller-append `[loop terminated: ...]` 告知父,不写 audit)。新增 `AuditKind::LoopIntervention`(**无 migration**,payload `{hit_count, verdict_kind, action, run_id}`)。`AlreadyPending`(LLM 并发 ask 占 slot)降级走原 hint。3 偏差(`run_id` 占位 future-proof / dispatch caller-append 不加 format 第 5 参 / worker 复用 `SubagentStatus::Incomplete` 不加 LoopTerminated 变体避免 migration)。1282 后端 + 728 前端 tests passed。完整 PRD 走 `.trellis/tasks/07-05-c2-loop-active-intervention/`,spec 见 [tool-contract "C2+ loop intervention"](../.trellis/spec/backend/tool-contract.md),决策见 [IMPLEMENTATION §4 2026-07-06](./IMPLEMENTATION.md#4-决策日志) |
...
> **已完成的 18 项**(B6 / B12 / B4 / C2 / C2+ / A7 / L2 / L1 / L3a / L3b PR1 / L3b PR2 / L3b PR3 / L3c / L3d / A2+ / A5+ / E1 / V2-2+)已从第三档移到 §1.2 已实施列表。
