...
| ~~A2+ P3~~ | ~~shell 执行期沙盒兜底(bubblewrap/overlayfs/firejail)~~ | **P3b 主体 2026-08-31 落地**,见 §1.2 A2+ P3b 行。判定层(A2+ P1+P2,07-04)之下的独立**限损层** — 判定错了也限损。P3a spike 同日通过定主路线(Landlock+seccomp 纯 Rust 零依赖);[spike PRD](../.trellis/tasks/08-31-a2-p3a-sandbox-spike/prd.md)。**余留 → P3c(UX 增强档)**:Plan 模式全命令沙盒化、只读/读写/放行三态 per-project 配置、bwrap 可选增强(namespace 路线,顺带收口 interop socket 残余面 — seccomp 无法按路径匹配 connect 的 sockaddr 指针、Landlock ABI v1 无 connect 位,v1 如实记录该残余面)、沙盒失败→Ask 升级闭环、网络白名单/egress 代理。源方案 [docs/_history/2026-08-28-a2-shell-classification.md](./_history/2026-08-28-a2-shell-classification.md) §4 |
