...
| **B6+** | subagent 多模型支持（B 动态选模型 / C UI 配置） | A=frontmatter `model:` 声明（07-03 落地，见 §1.2）；本条目剩余 = B（dispatch_subagent 加 `model` 参数 + `@@agent --model=`，优先级 dispatch>frontmatter>parent）+ C（Settings UI 配 per-agent 默认模型下拉，写回 frontmatter / builtin DB override，顺带解决 `model:` 写 id 不友好的问题）。动机：跨模型对抗性 review，抗同模型自确认偏差 |
