...
| **A2+ P3b** 执行期沙盒(Landlock+seccomp) | 08-31 | ReadOnly 档 shell 命令(前台 + 后台)在 Landlock(EXECUTE+写族 handled,读不控)+ seccomp(拦 socket AF_INET/AF_INET6)下执行,可写面 = worktree+/tmp/spill+config;四项触发与、能力探测 fail-open、kill-switch、审计 kind、设置面开关+徽标+额外可写目录编辑(spec [sandbox-executor](../.trellis/spec/backend/sandbox-executor.md),task `08-31-a2-p3b-sandbox-executor`;P3c 余 UX/bwrap 增强档) |
| **A2+ P3c** 沙盒 UX 增强档(三态 + Plan + 升级闭环) | 09-01 | `resolve_policy` 单一决策真源(capability → Yolo → 项目 off → kill-switch → Plan → 项目面,config 读惰性);per-project 三态档 `off/readwrite/readonly`(默认 readwrite = **全命令**进沙盒,行为变更),`projects.sandbox_policy` 列 + 设置面项目 scope 三态选择;Plan 模式重新暴露 shell 族(session 级只读面,面不可用回退工具过滤);前台 shell 失败升级闭环(面外写/断网 → Ask 卡 → 一次性不沙盒重跑,prefix-grant 记住);Plan/Edit/断网四变体指引;清 SBX-002/003/004(spec [sandbox-executor](../.trellis/spec/backend/sandbox-executor.md),task `09-01-a2-p3c-sandbox-ux`;余留 → follow-up:bwrap、网络白名单) |
| **A2+ P3d** 后台 shell 升级闭环 | 09-01 | 前台 §5.2 闭环补齐到 `run_background_shell`:面外失败的通知带 `EscalationOffer`(原始 tool_use_id + block 分类 + stderr 证据,registry 等待任务烘入);**下轮注入时**(drain 后、LLM 前)解析——prefix-grant 零卡直跑 / Ask 卡挂回原调用卡 → 批准一次性不沙盒重跑;Plan 门 + legacy 通知逐字节不动;ShellCard 审批区对"已有结果的后台卡"豁免 `!hasResult` 守卫(spec [sandbox-executor §11](../.trellis/spec/backend/sandbox-executor.md),task `09-01-a2-p3d-background-escalation`;余留 → bwrap、网络白名单/egress 代理) |
...
| **N1 首次引导**(方向调整:轻页面引导 + 内置诊断/引导/配置 LLM skills) | 09-15 | 聊天空状态四分态检测卡(无 provider/无模型/无默认/齐配,`config.loaded` 防闪变)+ 错误行「测试连接」行动点(直调 test_model,非 LLM 兜底);skill 新增**全局内置层**(GlobalBuiltin,所有会话 + `/` 面板可见,用户同名可覆盖,编译期 `include_str!` daemon-only 可用)承载三件套 `llm-setup`/`doctor`/`onboarding`;只读诊断双工具 `llm_diagnostics`(脱敏配置快照,**ProviderRow 禁序列化进工具输出**)/ `test_llm_connection`(1-token 实测,test_model_inner 签名收敛 db 参数);live 实跑 /doctor 全闭环(use_skill→快照→实测 944ms→问诊)(task `09-15-n1-onboarding-skills`,契约 spec tool-contract 12/18 + test-model-contract 签名注) |
| **N3 项目冷启动 /init**(GlobalBuiltin 第四件 `init`) | 09-18 | 新项目一条 `/init` 生成 `<project>/AGENTS.md`(LLM 归纳「项目结构」repo map 章节 + 构建/测试/约定模板),经 B5 loader 自动注入后续会话——解「指令文件手写 + 每 session grep 摸地形」;幂等三路径(不存在→全模板 / 含 `everlasting:repo-map` marker→`edit_file` 只换区块外字节零动 / 纯手写→不写文件报告);零机制新增(SKILL.md 指引资产 + `include_str!` 单行注册,工具面全复用),资产结构锁单测钉 marker/落点/反编造关键段(task `09-18-n3-project-init`) |
| **N2 checkpoint/revert 闭环**(悬空快照路线) | 09-20 | turn 边界文件快照 = DB 寻址悬空 git commit 链(git2 内存索引零接触用户分支/索引,写信号门四路触发,只读轮零成本)+ UI revert 两步命令(preview_token 封堵 TOCTOU + busy 拒 + foreign_delta 强制确认 + CheckpointReverted 审计)+ TurnCard「本轮 diff」读面(prev_seq 稀疏链)+ Settings 开关 fail-open;群聊评审 15 结论回填后四 PR 交付,AC1-11 全测(契约 spec [backend/checkpoint-contract](../.trellis/spec/backend/checkpoint-contract.md),task `09-20-n2-checkpoint-revert`) |
