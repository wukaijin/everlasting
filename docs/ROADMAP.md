...
| **L3d**  | **subagent frontmatter 加载器** | **让用户通过 `~/.config/everlasting/agents/*.md` + `<project>/.everlasting/agents/*.md` 定义自己的 sub-agent 类型;`dispatch_subagent` schema enum 自动扩展 + source tag(`user`/`project`/`builtin`);加载顺序 project > user > builtin,last-write-wins;Q1-Q8 设计已 grill 锁定(2026-06-25);详见 [docs/subagent-loader.md](./subagent-loader.md)** |
