...
- **MCP 部署面(2026-09-06 记,M2 收官后用户指认;✅ 路径② 2026-09-06 落地,task `09-06-gce-mcp-standalone`)**:M2 四工具是**仓库产物**(脚本 + node_modules + 指向源码检出内绝对路径的挂载配置),不随 daemon 分发——只有 daemon bin、无源码的机器上 MCP 层为零(仅剩 M0 裸 HTTP 原语)。这是 D1 的有意识取舍(v1 消费语境 = 本机 dev 工作流),但构成「任何宿主」终态的部署缺口。收口路径:①零成本接受(现状边界);②**单文件可执行(推荐,可独立小项提前做)**:bun/deno compile 打 standalone 二进制随 app 分发,安装器写 user-scope MCP 配置(免 node、绝对路径由安装期产生不进 git,JS 单实现保留,M2 纯逻辑零改动);③daemon 内置 streamable-http MCP endpoint(终态最干净,零外部依赖,也是远程暴露的天然载体;代价 = Rust 背协议 rmcp + 与 JS 引擎双实现,届时 JS 层降级为 dev 工具)。**落地记录(路径②)**:`scripts/group-chat-mcp-deploy.mjs` 一条命令 = bun compile standalone bin(落 XDG data 根 `bin/`,免 node/免 node_modules/免源码;sidecar `build-info` 记 git rev + 时间戳诊断 stale bin)→ ZCode user-scope 配置原位替换(备份 + 幂等);`--revert` 回 node 挂载(node 挂载保留为开发态默认)、`--uninstall` 清配置与 bin;引擎一行不改(CLI 壳误判根因与 argv[1] 哨兵解法见任务 research)。**follow-up 收窄为**:跨平台编译矩阵(macOS/Windows,交叉编译 flag 未验)+ Tauri app 分发 + 其他宿主(Claude Code/Cursor)配置写入。**路径③ ✅ 2026-09-14 落地**(task `09-14-gce-mcp-daemon-converge`,动机 = 内存:stdio 壳随宿主会话 spawn node / 98 MB bun bin,HTTP 化后零子进程,实测冒烟全链 daemon RSS 增量 ~1 MB):daemon 原生 `/mcp` streamable-HTTP 端点 —— wire 契约**反向提取自 SDK 1.30.0 源码**(非 rmcp:协议层手写,极简无状态 profile = JSON-only 响应 / GET 405 / 无 `mcp-session-id` / 版本 echo,lenient 版本头);八工具 1:1 平移 stdio 壳(wire schema + 4200 chars 预算锁同源),编排原语走 daemon 内部 `*_inner` 单源不经 HTTP 自绕,内置预设编译期 `include_str!` 嵌入 JSON 源文件(单一事实源保持),XDG 记账退役(rid 由 `session_active_request` 派生)。SDK 客户端冒烟 `scripts/group-chat-mcp-http-smoke.mjs`。**挂载切换 = P3 ✅ 2026-09-15**(别名 `everlasting-group-chat-http` 双挂验证宿主兼容 → 原名 `everlasting-group-chat` 条目原位换 HTTP 保工具前缀,双挂态备份 `config.json.p3-dual.bak`);**stdio 壳 / bun bin / deploy 脚本退役 = P4 ✅ 2026-09-15**(task `09-15-gce-mcp-stdio-retire`:删 6 个 JS 文件 + zod 依赖删除(SDK 保留——http-smoke 客户端消费;lockfile 收敛 pnpm 单源,npm 残留 package-lock.json 删)+ 机器 bin/sidecar;引擎与 Rust 行为零改动,JS 侧 MCP 层测试所有权移交 mcp.rs 单测 + http-smoke)。契约见 [DAEMON-API §6.5](./DAEMON-API.md)。
