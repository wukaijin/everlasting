...
## Session 89: 主对话放行 list 管理 UI

**Date**: 2026-07-01
**Task**: 07-01-permission-grant-list-ui (permission grant list management UI)
**Branch**: `main`

### Summary

「始终允许」放行此前无前端入口(只能直连 SQLite 或删 session CASCADE 清)。本次加管理 UI:ChatPanel header 钥匙按钮(`key` 图标,与 audit 按钮并列,`v-if currentSessionId`)→ PermissionGrantsModal(仿 AuditLogModal)列出当前 session 所有放行(tool/路径/前缀三 kind,带 glob/token 值),单行「撤销」按 PK 四元组精确删。后端新增 `list_session_tool_permissions` IPC + `revoke_tool_permission` 改签名(从 `(sid,tool)` 扩到 `(sid,tool,kind,value)`,D2 NULL 分支:None→`IS NULL` / Some→`= ?`,防止 `= NULL` 静默删 0 行)。撤销**立即生效**——check 路径每次查 DB(`check.rs:503/630/641`),无 main-session 缓存,DELETE 后下个 tool_use 即重新弹审批,无需 invalidate 信号。范围=session 级(非项目级);subagent worker 的 per-run cache 不进此表。

坑:trellis-check 那轮 Edit 把 store `rowKey` 的 `\u0000` 写成了字面 NUL 字节(0x00),git 把 `permissionGrants.ts` 当二进制(Bin,diff 失明)。用 perl 在字节层把 NUL 替换成 6 字符 JS 转义序列 `\u0000`(源文件纯 ASCII,运行时仍解析为 NUL)修复。trellis-check 判定可 commit;2 个非阻塞项(rowKey 已修;D4 path badge 用 emerald 而非 design 写的「蓝」,认定为合理 token 复用,不改)。

### Main Changes

- `db/permissions.rs` (+105): `PermissionGrantRow`(camelCase wire) + `list_tool_permissions` + `revoke` 改签名(D2 NULL 分支)
- `db/permissions_tests.rs` (+203): 5 测试(空列表 / 三 kind round-trip / D2 NULL 删命中 / sibling 保留 / session 隔离)
- `commands/permissions.rs` (+56): `list_session_tool_permissions` + `revoke_tool_permission` IPC
- `lib.rs`: 注册 2 命令
- `stores/permissionGrants.ts`(+test): store(loadForSession/revoke/refresh,rowKey `\u0000` 分隔)
- `components/permissions/PermissionGrantsModal.vue`: reka-ui Dialog modal(仿 audit,Vue 3.5 teleport scoped)
- `components/permissions/PermissionGrantItem.vue`(+test): 单行(kind badge + tool + value + 撤销)
- `ChatPanel.vue` (+70): 6 处接线(import / ref / watch 关闭 / 钥匙按钮 / modal 实例 / 样式)

### Git Commits

| Hash | Message |
|------|---------|
| `0f5b235` | feat(permissions): 主对话放行 list 管理 UI (list/revoke IPC + ChatPanel modal) |
| `5d8933d` | chore(task): archive 07-01-permission-grant-list-ui |

### Testing

- [OK] `cargo test --lib`: 1132 passed / 0 failed(含 5 新 db 测试)
- [OK] `vue-tsc --noEmit`: 无错误
- [OK] `pnpm vitest run`: 670 passed / 0 failed(含 store 7 + 组件 4)
- [ ] 手动 UI 实测(待用户): 三 kind 展示 + 撤销 + D1 立即生效

### Status

[OK] **Completed**(自动化全绿;手动 UI 实测待用户)

### Next Steps

- 用户启动 `pnpm tauri dev` 实测 UI(三 kind 展示 / 撤销立即生效)

## 2026-07-02 — 统一 8 个 modal 动画 (07-02-modal-motion-rhythm)

### Context

用户反馈 modal 动画"不自然"。排查发现 8 个 modal 共用一套 convention（150ms expo-out 进入），节奏"弹射急停"；且 PermissionGrantsModal 缺退出动画（关闭硬切 bug）。多轮实机迭代后定为：mask 无动画 + content scale 0.1↔1 + ease 曲线。

### Main Changes

- `style.css`: 新增 modal 专用 token `--duration-modal-in/out` (200/150ms) + `--ease-modal-in`/`--ease-accelerate` (均 = ease `cubic-bezier(0.25,0.1,0.25,1)`)
- 5 个 A 类 modal (reka-ui Dialog: Settings/Memory/Audit/MarkdownDetail/PermissionGrants): overlay 去 animation（mask 无动画）+ 删 fade keyframes；content zoom 改 scale 0.1↔1 + 新 token；PermissionGrants 补 zoom-out keyframe + `[data-state="closed"]`（修退出 bug）
- 3 个 B 类 modal (Vue Transition: ConfirmDialog/YoloConfirmModal/DiffModal): backdrop 改 opacity 始终 1（仅留 transition-duration 作 Vue 计时）；content scale 0.1↔1 + 新 token
- spec: `popover-pattern.md` modal convention 整段重写 + `design-tokens.md` 加 modal 专用 token 注记

### Git Commits

| Hash | Message |
|------|---------|
| `2cdf22d` | feat(modal): 统一 8 个 modal 动画 — mask 无动画 + scale 0.1↔1 + ease |
| (auto) | chore(task): archive 07-02-modal-motion-rhythm |

### Testing

- [OK] `pnpm build` (vue-tsc + vite)
- [OK] `pnpm vitest run`: 670 passed / 0 failed
- [OK] grep: 无残留 scale(0.96) / 旧 token / fade keyframes
- [OK] 用户实机确认: 进入/退出节奏自然（多轮迭代后定稿）
