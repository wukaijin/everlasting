...
| [Error Handling](./error-handling.md) | Error types (`LlmError` 5 类 + 8 thiserror + 2 手写 `AppError`),handling strategies (`?` + `From<E>` 传播 + `AppCommandError` wire shape),**17 RULE-A-*** 不变量(RULE-A-018 全局错误 toast 路由,2026-07-17 A5 scope B 落地,closed — `useErrorBus.routeByCategory` + `useToast` composable 驱动;主链路 36 处 IPC 错误展示不动) | Filled (2026-07-02 task `07-02-a5-error-handling-refine`; **2026-07-17 RULE-A-018 加**) |
...
| [Daemon Server Contract](./daemon-server.md) | axum `everlasting-daemon` 运维契约:`serve_daemon` + graceful shutdown 协议(signal → `SseRegistry::shutdown` 主动结束 SSE 长连接 → axum drain + `SHUTDOWN_GRACE_SECS` timeout 兜底)+ "新增 streaming endpoint 的 shutdown 检查" pattern(防 SSE/WebSocket/chunked 长连接卡住 graceful drain) | Filled (2026-07-24, task 07-24-daemon-graceful-shutdown) |
