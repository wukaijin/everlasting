...
| [Agent Loop Architecture](./agent-loop-architecture.md) | `run_chat_loop` 21-param shared entry point (production + test), CancellationGuard single-source via equivalence proof, B6 worker subagent pattern (nested run_chat_loop + **16** skip_persist gates [PR2a RULE-A-015 corrected from 18] + skip_session_active + is_worker 第 21 参 [PR2b RULE-A-014]), faithful-port drift hazard anti-pattern, new "Pattern: PR2a corrected PR1 over-broad skip_persist gate" (RULE-A-015), DEBT RULE-A-014 closed + RULE-A-016 open | Filled (2026-06-20, task 06-20-b6-pr2-subagent-persistence PR2b) |
