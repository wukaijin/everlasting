...
LLM 在主 chat loop 想切换当前 session 的 mode(plan → edit 落代码 / edit → plan 提议架构 / 高风险操作申请 yolo),目前无工具自请,只能返回 "please switch me to Edit mode" 让 user 手动 `Shift+Tab`。新 tool `request_mode_change` 让 LLM 走 `tool_use` 申请,user 通过 inline message card 二选一 allow / deny。**只服务主 session**,worker subagent 禁用(申请模式必须在 parent context 决策)。

### 2. Signatures

**ToolDef**(`tools/request_mode_change.rs::definition()`):

```json
{
  "name": "request_mode_change",
  "description": "Request the user to switch the current session's mode (edit/plan/yolo). ...",
  "input_schema": {
    "type": "object",
    "properties": {
      "target_mode": {
        "type": "string",
        "enum": ["edit", "plan", "yolo"],
        "description": "The mode the LLM is requesting."
      },
      "reason": {
        "type": "string",
        "maxLength": 500,
        "description": "Optional brief explanation shown on the card."
      }
    },
    "required": ["target_mode"]
  }
...
**IPC `resolve_mode_change`**(前端 → 后端,`commands/question.rs`):
...
**IPC `get_pending_interaction`**(前端 → 后端,新统一查询,supersedes `get_pending_question`):
...
**Tauri event `mode:change:request`**(后端 → 前端,`state.rs::AppHandleSink::emit_mode_change_request`):
...
- **Wire shape**:`ModeChangePayload` / `PendingInteractionEntry` snake_case(shared-struct exemption,跟 `ToolQuestionPayload` 同款:`ModeChangePayload` 与 LLM tool input 共享,LLM schema 是 snake_case)
- **Single-pending gate**:同 session 不能并发 2 个待决交互(跨 kind 互斥:`PendingInteraction::Question` 与 `PendingInteraction::ModeChange` 共用 `QuestionStore` HashMap entry,`register` 命中 `AlreadyPending` 返错误)
- **Session-switch preservation**:切 session 不取消 pending card,oneshot 全程保留,切回 session 通过 `get_pending_interaction` 恢复
- **Noop 短路**:`target_mode == current_mode` → tool 立即返 `{"noop": true, "current_mode": "..."}`(`is_error: false`),不弹 card、不发 IPC 事件、不 register(写 1 条 `mode_change_requested{noop: true}` audit 保留 LLM 申请痕迹)
- **Worker 禁用**:`STRUCTURALLY_DISABLED` 加 `request_mode_change`(`subagent/filter.rs`),worker tool list 不可见
- **并行 eligibility**:`is_parallel_eligible` 白名单不含(默认 false,整批 Serial,跟 `ask_user_question` / `dispatch_subagent` 同档)

### 4. Validation & Error Matrix

| Case | Result |
|------|--------|
| `target_mode` 不在 enum(`background` / `"yolo "` / `""`) | `is_error: true` + structured error,tool 入口 short-circuit,**不挂起** |
| `reason > 500 字符` | `is_error: true` + length error,同上 short-circuit |
| `target_mode == current_mode`(noop) | `{"noop": true, "current_mode": "..."}`(`is_error: false`),不挂起、不弹 card |
| `target_mode` 合法 + 同 session 已有 pending question | `QuestionStoreError::AlreadyPending` 等价错误(`is_error: true` + `"已有 pending interaction, 等当前处理完成"`) |
| `target_mode = "yolo"` + user 允许 + `is_running_as_root() = true` | `is_error: true` + `{"cancelled_by_user": true, "reason": "Cannot enable Yolo as root"}`,tool_result 仍是 cancelled marker |
| `target_mode = "yolo"` + user 拒绝 / 二次 modal 取消 | `is_error: true` + `{"cancelled_by_user": true}` |
| session cancel(token.cancelled() mid-wait) | `is_error: true` + `{"cancelled_by_session": true}`,`store.remove()` 清 entry |

### 5. Good / Base / Bad Cases

**Good — Plan → Edit,user 允许**:
LLM 在 plan mode 完成方案设计 → 申请 `target_mode=edit, reason="写落地代码"` → inline card 弹 → user 点"允许" → `resolve_mode_change(allow=true)` → `set_session_mode_internal` 落库 edit + `mode_changed` audit + `mode_change_allowed` audit → oneshot 解 → tool_result `{"allowed": true, "prev_mode": "plan", "new_mode": "edit"}` → LLM 下一轮 system prompt 体现 edit 行为约定(可写代码)。

**Good — Edit → Yolo,user 二次确认**:
LLM 在 edit mode 跑 destructive batch → 申请 `target_mode=yolo, reason="批量清理" ` → inline card 弹(target mode chip 红)→ user 点"允许" → `chatStore.requestSetMode(sid, "yolo")` → `pendingYoloConfirm` modal 弹(沿用 user 主动路径)→ user 二次"确认" → `confirmYolo(pendingResolve)` 调双 IPC:`set_session_mode` 落库 + `resolve_mode_change(true)` 解 oneshot → tool_result `{"allowed": true, ...}`。**root 时**modal "确认" 按钮 disabled + 红字 "Cannot enable Yolo as root",user 必须取消 modal → `cancelYolo(pendingResolve)` → `resolve_mode_change(false)` → 拒绝路径。

**Base — 拒绝路径**:
LLM 申请 `target_mode=plan` → card 弹 → user 点"拒绝" → `resolve_mode_change(allow=false)` → `mode_change_denied` audit → store resolve as Cancelled → tool_result `{"cancelled_by_user": true}`(`is_error: true`)→ LLM 看到继续 plan mode 工作(自行决定下一步)。

**Base — Noop**:
LLM 在 edit 模式申请 `target_mode=edit` → tool 入口 noop 检测 → 立即 `{"noop": true, "current_mode": "edit"}`(`is_error: false`),**不挂起、不弹 card** → LLM 看到继续 work。零 round-trip 开销。

**Bad — Yolo as root,user 强行点**:
LLM 申请 yolo → user 点"允许" → modal 弹,但"确认"按钮 disabled(root)→ user 取消 modal → `cancelYolo` → 拒绝路径。即使 user 强行绕过(理论上无可能,按钮 disabled),后端 `set_session_mode` IPC 再守一道(`is_running_as_root()` check)→ `InvalidRequest("Cannot enable Yolo as root")` → `resolve_mode_change_internal` 失败 → resolve as Cancelled → tool_result cancelled marker + `mode_change_denied{reason: "yolo_root_guard"}` audit。

**Bad — 双 pending 互斥**:
同 session 先 register 一个 ask_user_question pending → LLM 同 turn 再调 `request_mode_change` → tool 入口 `store.register(...)` 命中 `AlreadyPending` → `is_error: true` + `"已有 pending interaction, 等当前处理完成"`。第一个 pending 不动,正常 resolve。

### 6. Tests Required

- **`agent/tests_request_mode_change.rs`(5 cases,Phase E1)**:AC1 batch serializes with shell / AC7 noop skips card / AC9 already pending returns structured error / AC12 happy path records mode_change_requested audit / AC16 session cancel returns cancelled_by_session marker
- **`commands/tests_get_pending_interaction.rs`(4 cases,Phase E2)**:register Question → get Question / register ModeChange → get ModeChange / resolve → None / unknown session → None
- **`commands/tests_resolve_mode_change.rs`(5 cases,Phase E3)**:allow=true+plan / allow=true+yolo non-root / allow=true+yolo root guard denial / allow=false deny path / unknown session InvalidRequest
- **`tools/request_mode_change.rs::tests`(8 cases inline)**:validation empty / validation enum out of range / validation reason too long / noop / happy path / deny / cancel arm / AlreadyPending
- **`RequestModeChangeCard.test.ts`(23 cases)**:pending rendering / allow non-Yolo / allow Yolo → pendingYoloConfirm / deny / allowed-denied pills / targetMode color mapping / AC10 inline red line

### 7. Wrong vs Correct — 拦截路径(execute_tool_inner vs chat_loop loop)

跟 `dispatch_subagent` / `ask_user_question` 同款红线:

**Wrong**:`tools/mod.rs::execute_tool_inner` 的 `match` 里加 `request_mode_change => ...` 分支。
- Reason:这些 tool 是 **blocking user-interaction tool**,agent loop 必须等 user 决策(`tokio::select!` 挂 oneshot)。`execute_tool_inner` 在 `run_chat_loop` 内串行调用,没有 user-interaction 挂起语义。

**Correct**:`chat_loop.rs` tool 处理阶段特判 `if name == "request_mode_change"` → 直接调 `request_mode_change::execute_blocking(...)` → 返回 `(content, is_error, _update, exit_code)` 走与 `ask_user_question` 同一处理路径(`record_tool_executed_audit` + `emit_tool_result` + `ContentBlock::ToolResult` push + cancel 标记)。

`execute_tool_inner` 的 match 表**不加** `request_mode_change` 分支;`builtin_tools()` 注册 `request_mode_change::definition()` 仅供 LLM 看到工具描述(schema 自描述)。

### 8. Design Decisions

- **Yolo 二次 modal 守门(Decision D4, IMPLEMENTATION §4 2026-07-07)**:card "允许" + `targetMode === "yolo"` → `chatStore.requestSetMode(sid, "yolo")` → `pendingYoloConfirm` modal → `confirmYolo(pendingResolve)` 调双 IPC。`pendingResolve?: { sessionId, toolUseId, targetMode }` 是 `confirmYolo` / `cancelYolo` 的可选参数,**user 主动切 Yolo 路径(Shift+Tab / popover)ref 为 null,完全不动既有代码**。防御:`confirmYolo` / `cancelYolo` 在 fire resolve 前先清 `pendingResolveRequest.value = null`,防 stale ref 串到下次 user-initiated 切换。
- **QuestionStore 互斥(Decision D5)**:1 个 session 1 个 pending entry,跨 kind 互斥。`PendingInteraction` tagged enum + `register(sid, tuid, payload: PendingInteraction)` 共用同一 store,避免双 store 互不感知。
- **tool_result wire shape**:`Allowed(json) → execute_blocking` 构造 `(json!({"allowed": true, "prev_mode, new_mode}), false, _, None)`;`Cancelled → {"cancelled_by_user": true}`(`is_error: true`)。Session cancel 在 tool 内部 `tokio::select!` cancel arm 处理(不返 IPC,store.remove + `{"cancelled_by_session": true}`)。
- **3 类新 audit + `mode_changed` 不重复**:`mode_change_requested` (tool 入口)/ `mode_change_allowed` (IPC resolve allow)/ `mode_change_denied` (IPC resolve deny 或 root guard fail)。`mode_changed` 由 `db::update_session_mode` 副作用自动产生(走 `set_session_mode_internal` 路径),审计链完整且不重复。AuditKind 17 → 20。
- **IPC 统一化**:`get_pending_interaction` 新 IPC 替代 `get_pending_question`(后者标 `#[deprecated]`,`#[allow(deprecated)]` 在 `lib.rs` 注册处抑制警告)。`PendingInteractionEntry { kind, payload }` 顶层 `kind` tag 让前端 `entry.kind === "mode_change"` 不必先 parse 嵌套 enum。
