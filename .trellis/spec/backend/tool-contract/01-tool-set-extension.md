...
[memory §Scenario 2 / Pre-tool pitfall recall contract](../memory/scenario-am-p3-tool-recall.md)
...
[memory §Scenario 2 / Event-driven bypass reflection contract (P4)](../memory/scenario-am-p4-event-reflection.md)
...
| Command completes with non-zero exit | `shell` | Output + `[exit code: N]` marker, **is_error: false** (2026-09-02 — the exit code is data, not a tool failure; extends the "rg exit1 = not really an error" philosophy. Pre-change every non-zero exit was is_error: true and flipped the UI card into the red error state, punishing intentional exit-1 commands like grep/diff/test. Cancel / timeout / spawn failure remain is_error: true.) |
