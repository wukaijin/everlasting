...
|-------|-------------|--------|
| [State Management](./state-management.md) | Pinia store patterns, chat facade + streamController split, per-session message buffer LRU 20, SSE listener routing, **`useToast` composable** (2026-07-17 A5 scope B: module-level 单例 + MAX_CONCURRENT=3 + DEDUPE_WINDOW=5s + DEFAULT_TTL=5s + FIFO overflow + 4 category 颜色;AppShell 挂载 `ToastProvider`;`useErrorBus.routeByCategory` 5 stub 改 toast 路由) | Filled (2026-06, 6 UI/状态 bug 修复 PR + 8-PR3; **2026-07-17 A5 加 useToast composable section**) |
