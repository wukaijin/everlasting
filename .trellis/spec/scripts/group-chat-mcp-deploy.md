...
- `node scripts/group-chat-mcp-smoke.mjs [--bin <path>]` —— 双形态**同构**断言链(spawn + tools/list 八工具 + wire 预算 4200(09-13 status 加 wait_seconds/detail 后实测 4093)+ handler 错误链 + wait 探针 + list_presets 内置四 key 恒在);node 直连必须持续为默认回归路径
