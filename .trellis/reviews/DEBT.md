...
- **Level**: P3
- **Subsystem**: Frontend Subagent (cross Backend DB)
- **File**: `app/src/stores/subagentRuns.types.ts` (`SubagentStatus` type) + `app/src/components/chat/SubagentDrawer.vue` (`coerceStatus` + `STATUS_META`)
...
**最后更新**: 2026-06-23 by carlos — Session 65:**subagentRuns.ts 拆分**(task `06-23-06-23-split-subagent-runs`,refactor `b1c2a3d`,纯 file/import 重组零行为变更)。1416 行 → 3 文件:`subagentRuns.types.ts`(types + `SUBAGENT_EVENT_DEBOUNCE_MS`)+ `runAccumulator.ts`(`RunAccumulator` + `parseTranscriptJson` + `buildSectionsFromRaw` + chat_event 解析族)+ `subagentRuns.ts`(留 `coerceStatus` + store)。关键决策:`parseTranscriptJson` 跟 `RunAccumulator` 走 runAccumulator.ts(打破 store↔accumulator 循环依赖,`rebuildFromCache` 依赖它;非 task description 原说"留 store")。import 惯例严格遵循 `chat.types.ts` 先例(split-chat-types),11 外部文件 repoint。vue-tsc 0 error / vitest 475 pass(subagent 相关 161/161,0 error)/ build 绿。**file ref 更新**:RULE-FrontSubagent-005 `SubagentStatus` 迁 `subagentRuns.types.ts`;Session 64 NIT 的 `PermissionAskOutcome` 来源同步迁 `subagentRuns.types.ts`(NIT 本身——重复 literal 未 import 规范 type——仍未解决,follow-up)。4 pre-existing streamController errors(RULE-FrontTest-001 baseline,未触碰)。四段式 commit:refactor `b1c2a3d` + docs(debt) + archive + journal。
