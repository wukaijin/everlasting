...
| 对象 | 当前路径 | 来源 |
|---|---|---|
| SQLite DB | `~/.local/share/dev.everlasting.app/everlasting.db` | `state.rs:156-160` `app_data_dir()` ✅ |
| worktree | `~/.local/share/everlasting/worktrees/<project>/<session>` | `worktree.rs:40` env/home/`/tmp` ❌ |

- **identifier**(`tauri.conf.json:5`): `dev.everlasting.app`
- Tauri Linux `app_data_dir()` = `$XDG_DATA_HOME/dev.everlasting.app/` 或 `~/.local/share/dev.everlasting.app/`
- 根治后 worktree 落在 `~/.local/share/dev.everlasting.app/worktrees/`,跟 DB **同根**
...
路径 `~/.local/share/everlasting/worktrees/` → `~/.local/share/dev.everlasting.app/worktrees/`。
...
# 确认落点在 ~/.local/share/dev.everlasting.app/worktrees/<project>/<session>/
ls ~/.local/share/dev.everlasting.app/worktrees/
```
