...
- ✅ WP1:ROADMAP §5 删除"加 commit hash 引用"自相矛盾项,改为"列做了什么 + 时间,细节走链接";「不做的边界」重写为带 ❌/✅ 的自查点,与头部维护承诺一致。
- ✅ WP2:BACKLOG §1/§2/§4/§5.1 进度叙事全部收敛为「状态/排期归 ROADMAP」+ 链接;§5.1 冗余 commit hash、重复状态句删除;§3.1 上轮已收敛。
- ✅ WP3:CONTEXT BackgroundShell / Subagent / SubagentRun / Worker Worktree / MAX_TURNS / Context Compression Thresholds / Loop Detection / AuditKind / L1/L2/L3 / daemon 化进程模型收敛为"术语定义 + 链接";L1/L2/L3 与 daemon 段的实现细节/决策编号移入 ROADMAP/决策日志引用。
- ✅ WP4:ROADMAP §1.2 压缩 8 行最臃肿单元格(request_mode_change / L3b PR1/PR3 / L3d / B9+ / B6+ C / C2 / C2+ / E2 / D2① / D2②),删测试数/commit hash/实现机制,保留"做了什么+时间+链接";其余历史行按 PRD"可只处理最臃肿几行"保留。
- ✅ inventory 处置列 6 项全部 ✅,无 ⏸(附完成注记)。
- ✅ 链接校验 0 死链(40 文件,check-links.py;归档后脚本路径层级变化,复制到 `.trellis/tasks/_tmp-linkcheck/` 跑,跑完已清理)。
