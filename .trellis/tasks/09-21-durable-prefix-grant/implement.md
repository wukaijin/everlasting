...
- [ ] `shell_trust.rs`:新谓词 `grant_gate(cmd)` = `has_structural_
  metachar`(现状三符号)+ `\n` / `\r` + 单 `&` + `$(` + 反引号
  (`has_command_substitution` 复用);矩阵单测(含 `npm run\nrm -rf ~`)。
- [ ] 回灌三处既有读侧闸:`check_prefix_grant` 前置(permission.rs
...
- [ ] `db/migrations/schema.rs`:追加 `project_shell_grants` 建表
