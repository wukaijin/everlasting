...
struct PdfCIDFont<'a> {
    ...
    // EVERLASTING PATCH: /Encoding 是 Uni{GB,CNS,JIS,KS}-UCS2-{H,V} 之一。
    // 该家族码值本身就是 UCS-2(Unicode)码位,ToUnicode miss 时
    // decode_char 直接把码值当字符输出(见 decode_char 的 EVERLASTING
    // PATCH 分支)。
    ucs2_identity: bool,
...
// EVERLASTING PATCH: Adobe CMap 名 Uni{GB,CNS,JIS,KS}-UCS2-{H,V} 的 8 个
// 精确成员。规范里这族的 2 字节大端码值即 UCS-2 码位,解码不需要任何
// 外部 CMap 资源。精确匹配整名 —— 不能前缀匹配,否则会误吞 GB-EUC /
// GBK-EUC / B5pc 等老家族(那些 code ≠ Unicode,须真 CMap 资源才能解,
// 走降级路径)。
fn is_ucs2_cmap_name(name: &str) -> bool {
    matches!(
        name,
        "UniGB-UCS2-H"
            | "UniGB-UCS2-V"
...
fn get_unicode_map<'a>(doc: &'a Document, font: &'a Dictionary) -> Option<HashMap<u32, String>> {
    ...
    match to_unicode {
        Some(&Object::Stream(ref stream)) => {
            ...
            // EVERLASTING PATCH: 畸形 ToUnicode CMap 不再 unwrap panic ——
            // 视为无 unicode map,decode_char 自行降级(空串或 UCS2 直出)。
            let cmap = match adobe_cmap_parser::get_unicode_map(&contents) {
                Ok(cmap) => cmap,
                Err(e) => {
                    warn!("failed to parse ToUnicode CMap: {:?}", e);
                    return None;
                }
            };
            ...
            for (&k, v) in cmap.iter() {
                ...
                let mut i = 0;
                // EVERLASTING PATCH: 奇数长度/坏代理项不再 assert/unwrap
                // panic,跳过该条目(其余映射照常生效)。
                if v.len() % 2 != 0 {
                    continue;
                }
                while i < v.len() {
                ...
                let s = String::from_utf16(&be).unwrap_or_default();
        ...
        Some(&Object::Name(ref name)) => {
            ...
            // EVERLASTING PATCH: 非 Identity-H 的 ToUnicode Name 不再
            // todo! 崩进程 —— 视为无 unicode map,降级同上。
            if name != "Identity-H" {
                warn!("unsupported ToUnicode name: {:?} — ignoring", name);
            }
        }
        // EVERLASTING PATCH: 其余形态(数组等)不再 panic,同上降级。
        _ => {
            warn!("unsupported ToUnicode object: {:?} — ignoring", to_unicode)
        }
    }
    unicode_map
}
...
impl<'a> PdfCIDFont<'a> {
    fn new(doc: &'a Document, font: &'a Dictionary) -> PdfCIDFont<'a> {
        ...
        // EVERLASTING PATCH: ucs2_identity 标志由 is_ucs2_cmap_name 判定,
        // 供 decode_char 的 UCS-2 直出分支使用。
        let mut ucs2_identity = false;
        let encoding = match encoding {
            &Object::Name(ref name) => {
                ...
                if name == "Identity-H" || name == "Identity-V" {
                ...
                } else if is_ucs2_cmap_name(&name) {
                    // EVERLASTING PATCH: Uni*-UCS2-{H,V} 家族 —— 码值即
                    // UCS-2 码位,codespace/CID 映射与 Identity 同构
                    // (2 字节 codespace + cid identity),码值原样进
                    // decode_char。
                    dlog!("UCS2 identity encoding {}", name);
                    ucs2_identity = true;
                    ByteMapping {
                        codespace: vec![CodeRange {
                            width: 2,
                    ...
                    // EVERLASTING PATCH: 上游此处 panic!("unsupported
                    // encoding")。WPS/Word 中文导出常见 GB-EUC/B5pc 等老
                    // 家族 —— code ≠ Unicode,须真 CMap 资源才能解(本
                    // crate 不带)。降级:空 codespace 令 next_char 恒返回
                    // None,该字体字形被跳过,同页其余字体照常提取;
                    // 调用方(doc_extract.rs)的 turn 不因一个字体而死。
                    warn!(
                        "unsupported Type0 encoding {} — skipping this font's glyphs",
                        name
                    );
                    ByteMapping {
                        codespace: vec![],
            ...
            &Object::Stream(ref stream) => {
                let contents = get_contents(stream);
                dlog!("Stream: {}", String::from_utf8(contents.clone()).unwrap());
                // EVERLASTING PATCH: 内嵌 CMap 解析失败不再 unwrap panic,
                // 同上降级为跳过该字体。
                adobe_cmap_parser::get_byte_mapping(&contents).unwrap_or(ByteMapping {
                    codespace: vec![],
                    cid: vec![],
                })
            }
            // EVERLASTING PATCH: 非 Name/Stream 形态(数组等)不再 panic,同上降级。
            _ => {
                warn!(
                    "unsupported Type0 encoding {:?} — skipping this font's glyphs",
                    encoding
...
impl<'a> PdfFont for PdfCIDFont<'a> {
    ...
    fn next_char(&self, iter: &mut Iter<u8>) -> Option<(CharCode, u8)> {
        // EVERLASTING PATCH: 空 codespace = 降级字体(未知编码家族)。
        // 立即返回 None 跳过该字体全部字形(不吞 string 字节);外层
        // for 循环自然结束,同页其余字体不受影响。
        if self.encoding.codespace.is_empty() {
            return None;
        }
    ...
    fn decode_char(&self, char: CharCode) -> String {
        ...
        if let Some(s) = s {
            s.clone()
        } else if self.ucs2_identity {
            // EVERLASTING PATCH: Uni*-UCS2-{H,V} 家族且 ToUnicode miss
            // (非嵌入 CJK 字体的常态,如 STSong-Light)—— 码值本身即
            // UCS-2 码位,直出该字符。上游此处返回空串,会把整个字体
            // 的字符静默吞掉。代理区(0xD800-0xDFFF)不是合法标量,
            // from_u32 返回 None → 空串,与上游 miss 行为一致。
            char::from_u32(char)
                .map(|c| c.to_string())
                .unwrap_or_default()
        } else {
