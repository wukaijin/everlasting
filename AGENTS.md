...
- **群聊审议驱动(GCE-M1)**:`node scripts/group-chat-run.mjs run --preset review --topic "..."` — 一条命令召集跨模型审议(建群→发题→轮询→导转录到 `out/`);`projects`/`models`/`presets` 三内省查建群信息,`--dry-run` 零成本冒烟,模型引用只认 UUID(名字由脚本解析)。LLM 调用方指引见 `.agents/skills/group-chat/`。改了群聊编排/prompt 后跑一场 live 验证质量(一场 5-15min、数十万 token,慎用)。引擎纯函数单测:`node --test scripts/group-chat-run.test.mjs`(8 用例;vitest 不收 scripts/,走 node 内建 runner)。
