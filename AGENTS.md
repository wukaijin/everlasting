...
- **用户群聊预设(GCE-P1,09-12)**:用户预设存 DB `group_chat_presets` 表(Settings「群聊预设」页可管理,模型引用 UUID;IPC 契约见 docs/DAEMON-API.md §6.4),GUI 定时任务/建群弹窗的 preset 选择区叠加在内置四档之后;**内置四档仍是 `scripts/group-chat-presets.json` 单一事实源**(只读),M1/MCP 暂只认内置(用户预设消费是 P2)。任务 config 里的 `preset_key` 是纯出处字段,fire 零读取(快照语义)。
