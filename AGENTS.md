...
- **群聊审议驱动(GCE-M1)**:`node scripts/group-chat-run.mjs run --preset review --topic "..."` — 一条命令召集跨模型审议(建群→发题→轮询→导转录到 `out/`);`projects`/`models`/`presets` 三内省查建群信息,`--dry-run` 零成本冒烟,`--token-budget <n>` 声明预算帽(四计费字段口径,超限轮头停 `stop_reason=budget`;转录统计段带 per-speaker 核算),模型引用只认 UUID(名字由脚本解析)。LLM 调用方指引见 `.agents/skills/group-chat/`。改了群聊编排/prompt 后跑一场 live 验证质量(一场 5-15min、数十万 token,慎用)。引擎纯函数单测:`node --test scripts/group-chat-run.test.mjs`(16 用例;vitest 不收 scripts/,走 node 内建 runner)。
- **群聊 MCP 接口(GCE-M2)**:宿主 agent 经**用户级** MCP 配置(`~/.zcode/cli/config.json` 的 `mcp.servers`,2026-09-06 从仓库级 `.agents/mcp.json` 迁移——workspace 作用域跨项目不可见)挂七工具(M3 起控制面:`start_discussion`/`discussion_status`/`discussion_result`/`cancel_discussion`/`interrupt_discussion` 收束打断/`inject_message` 注入;09-11 起 `list_models` 只读内省可用模型,名字/UUID 皆可作引用),server = `scripts/group-chat-mcp.mjs`,与 M1 引擎共享实现层;args 为绝对路径,换机器须改。冒烟:`node scripts/group-chat-mcp-smoke.mjs`(非 live 零成本:spawn + tools/list + 预算 + 错误链;`--live` 烧真 token 走全链)。单测:`node --test scripts/group-chat-mcp.test.mjs`(19 用例,含 AC4 wire 预算锁 3500 字符——09-11 加 `list_models` 后实测 3403)。依赖装在 `scripts/package.json`(SDK+zod,勿装到根)。部署面(09-06-gce-mcp-standalone):`node scripts/group-chat-mcp-deploy.mjs` 一条命令把 MCP server 打成 standalone bin(免 node/免 node_modules/免源码)并写 user-scope 挂载,`--revert` 回 node 挂载(开发态默认)、`--uninstall` 清理;对 bin 冒烟:`node scripts/group-chat-mcp-smoke.mjs --bin ~/.local/share/dev.everlasting.app/bin/everlasting-group-chat-mcp`。
...
- **用户群聊预设(GCE-P1,09-12)**:用户预设存 DB `group_chat_presets` 表(Settings「群聊预设」页可管理,模型引用 UUID;IPC 契约见 docs/DAEMON-API.md §6.4),GUI 定时任务/建群弹窗的 preset 选择区叠加在内置四档之后;**内置四档仍是 `scripts/group-chat-presets.json` 单一事实源**(只读),M1/MCP 暂只认内置(用户预设消费是 P2)。任务 config 里的 `preset_key` 是纯出处字段,fire 零读取(快照语义)。内置四档可被 DB 覆盖行原位覆盖(GCE-P1b:`builtin_key` 列,Settings 该页「覆盖编辑」;删除覆盖行即恢复源码定义;M1/MCP 仍只认 JSON)。
