...
- **群聊 MCP 接口(GCE-M2)**:宿主 agent 经**用户级** MCP 配置(`~/.zcode/cli/config.json` 的 `mcp.servers`,2026-09-06 从仓库级 `.agents/mcp.json` 迁移——workspace 作用域跨项目不可见)挂四工具(`start_discussion`/`discussion_status`/`discussion_result`/`cancel_discussion`,server = `scripts/group-chat-mcp.mjs`,与 M1 引擎共享实现层;args 为绝对路径,换机器须改)。冒烟:`node scripts/group-chat-mcp-smoke.mjs`(非 live 零成本:spawn + tools/list + 预算 + 错误链;`--live` 烧真 token 走全链)。单测:`node --test scripts/group-chat-mcp.test.mjs`(13 用例,含 AC4 wire 预算锁 2300 字符)。依赖装在 `scripts/package.json`(SDK+zod,勿装到根)。
