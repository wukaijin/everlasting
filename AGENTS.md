...
- **群聊 MCP 接口(GCE-M2)**:宿主 agent 经**用户级** MCP 配置(`~/.zcode/cli/config.json` 的 `mcp.servers`,2026-09-06 从仓库级 `.agents/mcp.json` 迁移——workspace 作用域跨项目不可见)挂六工具(M3 起增控制面:`start_discussion`/`discussion_status`/`discussion_result`/`cancel_discussion`/`interrupt_discussion` 收束打断/`inject_message` 注入,server = `scripts/group-chat-mcp.mjs`,与 M1 引擎共享实现层;args 为绝对路径,换机器须改)。冒烟:`node scripts/group-chat-mcp-smoke.mjs`(非 live 零成本:spawn + tools/list + 预算 + 错误链;`--live` 烧真 token 走全链)。单测:`node --test scripts/group-chat-mcp.test.mjs`(18 用例,含 AC4 wire 预算锁 3200 字符——gce-m4c 加 `token_budget` 参后实测 3115)。依赖装在 `scripts/package.json`(SDK+zod,勿装到根)。部署面(09-06-gce-mcp-standalone):`node scripts/group-chat-mcp-deploy.mjs` 一条命令把 MCP server 打成 standalone bin(免 node/免 node_modules/免源码)并写 user-scope 挂载,`--revert` 回 node 挂载(开发态默认)、`--uninstall` 清理;对 bin 冒烟:`node scripts/group-chat-mcp-smoke.mjs --bin ~/.local/share/dev.everlasting.app/bin/everlasting-group-chat-mcp`。
