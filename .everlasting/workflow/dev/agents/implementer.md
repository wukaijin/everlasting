...
推进 task.items 里 `status=in_progress` 的项(用 `update_checklist` 改写 `task.json.items`,**非** loop-local Vec)。改动前 read 相关 spec,改完 `cargo check` + `cargo test --lib` 确认不破坏现有行为。
...
- 不要重复跑 cargo build(R2:复用 cargo cache)