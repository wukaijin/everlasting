...
- **`model` 参数 = 该模型的 display_name —— 强制必传**(per-dispatch override,reviewer.md 的 `model:` 留空就是为此)。**漏传 → reviewer 静默继承父 session 的默认模型 → 多个 reviewer 跑成同一个模型 → "多模型分歧"的整个价值落空。这属于执行错误。** 上一轮评审(session 6b313ce4)正是因为两个 reviewer 都漏传 `model`,实际跑了同一个默认模型,事后从 DB `model_display` NULL 才发现(现已回填实际继承的模型,但漏传本身仍要避免)
