...
- **`model` 参数 = 该模型的 display_name —— 强制必传**(per-dispatch override,reviewer.md 的 `model:` 留空就是为此)。**漏传 → reviewer 静默继承父 session 的默认模型 → 多个 reviewer 跑成同一个模型 → "多模型分歧"的整个价值落空。这属于执行错误。** 这不是理论风险:曾有一轮评审里所有 reviewer 都漏传了它,实际跑成同一个默认模型,事后核对运行记录才发现 —— 派发前逐个确认 `model` 已显式传入。
