...
use super::super::ask::ask_path;
use super::super::audit::{record_audit, AuditKind};
use super::super::store::PermissionStore;
use super::super::types::{Decision, PermissionContext};
...
pub async fn check(
    ...
    if let Some(reason) = super::super::dangerous::is_kill_listed(tool_name, tool_input) {
        let critical = true;
                ...
                if crate::projects::boundary::is_within_root(&ctx.project_main_path, &abs_path) {
                    abs_path.canonicalize().is_ok_and(|canon| {
                        !crate::projects::boundary::is_within_root(&ctx.project_main_path, &canon)
                            && super::super::sensitive::is_sensitive_path(&canon)
                    })
                } else {
                    super::super::sensitive::is_sensitive_path(&abs_path)
                };
            if sensitive_hit {
        ...
        ToolKind::Path => {
            ...
            match path_str {
                Some(p) => {
                    ...
                    if matches!(tool_name, "read_file" | "grep" | "glob" | "list_dir")
                        && super::super::sensitive::is_trusted_external(&abs_path)
                    {
                        let _ = record_audit(
                            db,
                            ctx,
        ...
        ToolKind::Shell => {
            ...
            if !super::super::shell_trust::has_structural_metachar(cmd) {
                if let Ok(true) = check_prefix_grant(
                    db,
                    &ctx.session_id,
                    &super::super::shell_trust::first_token_for_allow_always(cmd),
                )
                .await
                {
                    let _ =
                ...
                // also gated.
                if let Some(cache) = &ctx.run_grants {
                    let first_token = super::super::shell_trust::first_token_for_allow_always(cmd);
            ...
            match super::super::shell_trust::classify_prefix(cmd) {
                super::super::shell_trust::ShellTrust::ReadOnly => {
                ...
                super::super::shell_trust::ShellTrust::SideEffect => {
                ...
                super::super::shell_trust::ShellTrust::Ask => {
...
pub(crate) fn match_value_for_allow_always(
    ...
    match classify_tool(tool_name) {
        ...
        ToolKind::Shell => {
            // first token
            let prefix = super::super::shell_trust::first_token_for_allow_always(path_or_cmd);
            ("prefix", Some(prefix))
        }
        ToolKind::WebFetch => {
